...
let idCount = 0;
exports.getQuotes = (count) => Array.from({ length: count }, (v, k) => k).map(() => {
    const random = exports.quotes[Math.floor(Math.random() * exports.quotes.length)];
    const custom = {
        id: `quote-${idCount++}`,
...
exports.getAuthors = (count) => Array.from({ length: count }, (v, k) => k).map(() => {
    const random = exports.authors[Math.floor(Math.random() * exports.authors.length)];
    const custom = {
        id: `author-${idCount++}`,
        name: random.name,
...
exports.authorQuoteMap = exports.authors.reduce((previous, author) => (Object.assign({}, previous, { [author.name]: getByAuthor(author, exports.quotes) })), {});
