...
export async function callLoad(p) {
    ...
    let pm: PBase = {};
    let data = await ft<ReturnData<GridInfo<server.Menu>>>(ap.GET__api_Menu, pm);
...
export async function callEdit(id) {
    ...
    let tm = { id };
    let pm = doWKPA(tm);
    let data = await ft<ReturnData<server.Menu>>(ap.GET__api_Menu_Item, pm);
    mask_off();
    if (data.state > 0) {
        alert(data.message);
        return;
...
export async function callRemove(id, p) {
    ...
    let tm = { id };
    let pm = doWKPA(tm);
    let data = await ft<ReturnBase>(ap.POST__api_Menu_Remove, pm);
    mask_off();
    if (data.state > 0) {
...
export async function submitData(id: string | number, edit_type: IEditType, data: server.Menu) {
    ...
    if (edit_type == IEditType.modify) {
        ...
        let data = await ft<ReturnUpdate<any>>(ap.POST__api_Menu_Update, pm);
        ...
        let data = await ft<ReturnUpdate<any>>(ap.POST__api_Menu, pm);
...
export const addState = () => {
    let r: { type: string, data: server.Menu } = {
        type: ac.insert,
        data: { //新增值初始化
            role_array: [],
            parent_menu_id: 0,
...
//reduces
const page_grid = (state: GridInfo<server.Menu> = { rows: [] }, action): GridInfo<server.Menu> => {
    switch (action.type) {
        case ac.load:
            return action.data;
        case ac.page:
...
const field = (state = { role_array: [] }, action): server.Menu => {
    switch (action.type) {
        case ac.insert:
            return action.data
        case ac.modify:
...
const kfield = (state = { role_array: [] }, action): server.Menu => {
    switch (action.type) {
        case ac.insert:
            return action.data;
        case ac.modify:
