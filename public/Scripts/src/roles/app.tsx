...
interface GridProps {
    ...
    edit_type?: IEditType,
    menu_name?: string,
    page_grid?: GridInfo<server.LogRole>,
    callEdit?: Function,
    callRemove?: Function,
    callPage?: Function,
    callLoad?: Function,
        ...
        out_html = <div style={{ display: show }}>
            ...
            <table className="table-list table-hover table-striped">
                ...
                <tbody>
                    {
                        row_data.map((item, i) => {
                            return <tr key={item.role_id}>
                                <td className="item-edit"><PWButton className="hover-danger oi" title={lang.delete} dataGlyph="trash" onClick={this.callRemove.bind(this, item.role_id)}></PWButton></td>
                                <td className="item-edit"><PWButton className="hover-success oi" title={lang.modify} dataGlyph="pencil" onClick={this.callEdit.bind(this, item.role_id)}></PWButton></td>
                                <td className="text-left">{item.role_id}</td>
...
interface EditProps {
    edit_type?: IEditType
    field?: server.LogRole
    kfield?: server.LogRole
    exist?: boolean
    oper_id?: string
    ...
    render() {
        ...
        out_html = <div style={{ display: show }}>
            ...
            <form className="form-list" onSubmit={this.submit}>
                ...
                <dl className="field">
                    <dt className="col-2">權限名稱</dt>
                    <dd className="col-10">
                        <InputText
                            inputClassName="form-element"
                            onChange={this.chgFldVal.bind(this, 'role_name')}
                            value={pp.field.role_name}
                            required={true}
                            maxLength={30}
                        />
                    </dd>
