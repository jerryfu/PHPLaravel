...
export async function callLoad(p) {
    ...
    let pm: PBase = {};
    let data = await ft<ReturnData<server.ParamVideoPush>>(ap.GET__api_VideoPush, pm);
...
export async function submitData(edit_type: IEditType, md: server.VideoLink) {

    let tm = { md };
    let pm = doWKPA(tm);

    let data = await ft<ReturnUpdate<any>>(ap.POST__api_VideoPush, md);
...
const field = (state = {}, action): server.ParamVideoPush => {
    switch (action.type) {
        case ac.load:
            return action.data
        case ac.chgFdlVal:
