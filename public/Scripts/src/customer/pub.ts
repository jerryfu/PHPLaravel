...
export async function callLoad(p) {
    ...
    let pm: PBase = {};
    let data = await ft<ReturnData<GridInfo<server.Customer>>>(ap.GET__api_Customer, pm);
...
export async function callEdit(id) {
    ...
    let tm = { id };
    let pm = doWKPA(tm);
    let data = await ft<ReturnData<server.Customer>>(ap.GET__api_Customer_Item, pm);
    mask_off();
    if (data.state > 0) {
        alert(data.message);
        return;
...
export async function callRemove(id, p) {
    ...
    let tm = { id };
    let pm = doWKPA(tm);
    let data = await ft<ReturnBase>(ap.POST__api_Customer_Remove, pm);
    mask_off();
    if (data.state > 0) {
...
export async function submitData(id: string | number, edit_type: IEditType, data: server.Customer) {
    ...
    if (edit_type == IEditType.modify) {
        ...
        let data = await ft<ReturnUpdate<any>>(ap.POST__api_Customer_Update, pm);
        ...
        let data = await ft<ReturnUpdate<any>>(ap.POST__api_Customer, pm);
...
export const addState = () => {
    let r: { type: string, data: server.Customer } = {
        type: ac.insert,
        data: { //新增值初始化
            customer_id: guid(),
            gender:'M',
...
//reduces
const page_grid = (state: GridInfo<server.Customer> = { rows: [] }, action): GridInfo<server.Customer> => {
    switch (action.type) {
        case ac.load:
            return action.data;
        case ac.page:
...
const field = (state = {}, action): server.Customer => {
    switch (action.type) {
        case ac.insert:
            return action.data
        case ac.modify:
...
const kfield = (state = {}, action): server.Customer => {
    switch (action.type) {
        case ac.insert:
            return action.data;
        case ac.modify:
