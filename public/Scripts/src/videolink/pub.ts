...
export async function callLoad(p) {
    ...
    let pm: PBase = {};
    let data = await ft<ReturnData<GridInfo<server.VideoLink>>>(ap.GET__api_VideoLink, pm);
...
export async function callEdit(id) {
    ...
    let tm = { id };
    let pm = doWKPA(tm);
    let data = await ft<ReturnData<server.VideoLink>>(ap.GET__api_VideoLink_Item, pm);
    mask_off();
    if (data.state > 0) {
        alert(data.message);
        return;
...
export async function callRemove(id, p) {
    ...
    let tm = { id };
    let pm = doWKPA(tm);
    let data = await ft<ReturnBase>(ap.POST__api_VideoLink_Remove, pm);
    mask_off();
    if (data.state > 0) {
...
export async function submitData(id: string | number, edit_type: IEditType, data: server.VideoLink) {
    ...
    if (edit_type == IEditType.modify) {
        ...
        let data = await ft<ReturnUpdate<any>>(ap.POST__api_VideoLink_Update, pm);
        ...
        let data = await ft<ReturnUpdate<any>>(ap.POST__api_VideoLink, pm);
...
export const addState = () => {
    let r: { type: string, data: server.VideoLink } = {
        type: ac.insert,
        data: { //新增值初始化
            video_id: 0,
            state: 'A',
...
//reduces
const page_grid = (state: GridInfo<server.VideoLink> = { rows: [] }, action): GridInfo<server.VideoLink> => {
    switch (action.type) {
        case ac.load:
            return action.data;
        case ac.page:
...
const field = (state = {}, action): server.VideoLink => {
    switch (action.type) {
        case ac.insert:
            return action.data
        case ac.modify:
...
const kfield = (state = {}, action): server.VideoLink => {
    switch (action.type) {
        case ac.insert:
            return action.data;
        case ac.modify:
