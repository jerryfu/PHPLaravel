...
export async function callLoad(p) {
    ...
    let pm: PBase = {};
    let data = await ft<ReturnData<GridInfo<server.Sponsor>>>(ap.GET__api_Sponsor, pm);
...
export async function callEdit(id) {
    ...
    let tm = { id };
    let pm = doWKPA(tm);
    let data = await ft<ReturnData<server.Sponsor>>(ap.GET__api_Sponsor_Item, pm);
    mask_off();
    if (data.state > 0) {
        alert(data.message);
        return;
...
export async function callRemove(id, p) {
    ...
    let tm = { id };
    let pm = doWKPA(tm);
    let data = await ft<ReturnBase>(ap.POST__api_Sponsor_Remove, pm);
    mask_off();
    if (data.state > 0) {
...
export async function submitData(id: string | number, edit_type: IEditType, data: server.Sponsor) {
    ...
    if (edit_type == IEditType.modify) {
        ...
        let data = await ft<ReturnUpdate<any>>(ap.POST__api_Sponsor_Update, pm);
        ...
        let data = await ft<ReturnUpdate<any>>(ap.POST__api_Sponsor, pm);
...
export const addState = () => {
    let r: { type: string, data: server.Sponsor } = {
        type: ac.insert,
        data: { //新增值初始化
            sponsor_id: 0,
            state: 'A',
...
//reduces
const page_grid = (state: GridInfo<server.Sponsor> = { rows: [] }, action): GridInfo<server.Sponsor> => {
    switch (action.type) {
        case ac.load:
            return action.data;
        case ac.page:
...
const field = (state = {}, action): server.Sponsor => {
    switch (action.type) {
        case ac.insert:
            return action.data
        case ac.modify:
...
const kfield = (state = {}, action): server.Sponsor => {
    switch (action.type) {
        case ac.insert:
            return action.data;
        case ac.modify:
