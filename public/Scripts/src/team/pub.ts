...
export async function callLoad(p) {
    ...
    let pm: PBase = {};
    let data = await ft<ReturnData<GridInfo<server.Team>>>(ap.GET__api_Team, pm);
...
export async function callEdit(id) {
    ...
    let tm = { id };
    let pm = doWKPA(tm);
    let data = await ft<ReturnData<server.Team>>(ap.GET__api_Team_Item, pm);
    mask_off();
    if (data.state > 0) {
        alert(data.message);
        return;
...
export async function callRemove(id, p) {
    ...
    let tm = { id };
    let pm = doWKPA(tm);
    let data = await ft<ReturnBase>(ap.POST__api_Team_Remove, pm);
    mask_off();
    if (data.state > 0) {
...
export async function submitData(id: string | number, edit_type: IEditType, data: server.Team) {
    ...
    if (edit_type == IEditType.modify) {
        ...
        let data = await ft<ReturnUpdate<any>>(ap.POST__api_Team_Update, pm);
        ...
        let data = await ft<ReturnUpdate<any>>(ap.POST__api_Team, pm);
        ...
        if (data.state == err_code.HasErrList) {
            ...
            tosMessage('', data.message, ToastrType.error)
            //return dispatch => {
            //    dispatch({ type: ac.submitNot, field: tm.md });
            //}
...
export const addState = () => {
    let r: { type: string, data: server.Team } = {
        type: ac.insert,
        data: { //新增值初始化
            team_id: 0,
            state: 'A'
...
//reduces
const page_grid = (state: GridInfo<server.Team> = { rows: [] }, action): GridInfo<server.Team> => {
    switch (action.type) {
        case ac.load:
            return action.data;
        case ac.page:
...
const field = (state = {}, action): server.Team => {
    switch (action.type) {
        case ac.insert:
            return action.data
        case ac.modify:
...
const kfield = (state = {}, action): server.Team => {
    switch (action.type) {
        case ac.insert:
            return action.data;
        case ac.modify:
