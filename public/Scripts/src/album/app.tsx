...
interface GridProps {
    ...
    edit_type?: IEditType,
    menu_name?: string,
    page_grid?: GridInfo<server.Album>,
    callEdit?: Function,
    callRemove?: Function,
    callPage?: Function,
    callLoad?: Function,
        ...
        out_html = <div style={{ display: show }}>
            ...
            <table className="table-list table-hover table-striped">
                ...
                <tbody>
                    {
                        row_data.map((item, i) => {
                            return <tr key={item.album_id}>
                                <td className="item-edit">
                                    <PWButton className="hover-danger oi" title={lang.delete} dataGlyph="trash" onClick={this.callRemove.bind(this, item.album_id)}>
                                    </PWButton>
                                </td>
                                <td className="item-edit">
                                    <PWButton className="hover-success oi" title={lang.modify} dataGlyph="pencil" onClick={this.callEdit.bind(this, item.album_id)}>
                                    </PWButton>
                                </td>
                                <td className="text-left">{item.title}</td>
                                <td>{stdDate(item.set_date)}</td>
                                <td>{item.sort}</td>
        ...
        out_html = <div style={{ display: show }}>
            ...
            <form className="form-list" onSubmit={this.submit}>
                ...
                <dl className="field">
                    ...
                    <dd className="col-9">
                        <AreaText
                            ...
                            rows={4}
                        />
                        <small className="text-danger">* 不超過45個字</small>
                    </dd>
                </dl>
                ...
                <dl className="field">
                    ...
                    <dd className="col-5">
                        {pp.edit_type == IEditType.modify ? <FileUpBtn id={pp.field.album_id} fileKey="album_list" title="圖片上傳" is_img={true} />
