...
export async function callLoad(p) {
    ...
    let pm: PBase = {};
    let data = await ft<ReturnData<GridInfo<server.LogAccount>>>(ap.GET__api_LogAccount, pm);
...
export async function callEdit(id) {
    ...
    let tm = { id };
    let pm = doWKPA(tm);
    let data = await ft<ReturnData<server.LogAccount>>(ap.GET__api_LogAccount_Item, pm);
    mask_off();
    if (data.state > 0) {
        alert(data.message);
        return;
...
export async function callRemove(id, p) {
    ...
    let tm = { id };
    let pm = doWKPA(tm);
    let data = await ft<ReturnBase>(ap.POST__api_LogAccount_Remove, pm);
    mask_off();
    if (data.state > 0) {
...
export async function submitData(id: string | number, edit_type: IEditType, data: server.LogAccount) {
    ...
    if (edit_type == IEditType.modify) {
        ...
        let data = await ft<ReturnUpdate<any>>(ap.POST__api_LogAccount_Update, pm);
        ...
        let data = await ft<ReturnUpdate<any>>(ap.POST__api_LogAccount, pm);
...
export const addState = () => {
    let r: { type: string, data: server.LogAccount } = {
        type: ac.insert,
        data: { //新增值初始化
        }
    }
...
//reduces
const page_grid = (state: GridInfo<server.LogAccount> = { rows: [] }, action): GridInfo<server.LogAccount> => {
    switch (action.type) {
        case ac.load:
            return action.data;
        case ac.page:
...
const field = (state = {}, action): server.LogAccount => {
    switch (action.type) {
        case ac.insert:
            return action.data
        case ac.modify:
...
const kfield = (state = {}, action): server.LogAccount => {
    switch (action.type) {
        case ac.insert:
            return action.data;
        case ac.modify:
