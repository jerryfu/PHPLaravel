...
export async function callLoad(p) {
    ...
    let pm: PBase = {};
    let data = await ft<ReturnData<server.ParamAboutUs>>(ap.GET__api_CustomerInfo, pm);
...
export async function submitData(edit_type: IEditType, md: server.ParamAboutUs) {

    let tm = { md };
    let pm = doWKPA(tm);

    let data = await ft<ReturnUpdate<any>>(ap.POST__api_CustomerInfo, md);
...
const field = (state = {}, action): server.ParamAboutUs => {
    switch (action.type) {
        case ac.load:
            return action.data
        case ac.chgFdlVal:
...
const kfield = (state = {}, action): server.ParamAboutUs => {
    switch (action.type) {
        case ac.load:
            return action.data
        case ac.submitOK:
