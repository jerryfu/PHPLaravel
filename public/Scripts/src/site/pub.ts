...
export async function callLoad(p) {
    ...
    let pm: PBase = {};
    let data = await ft<ReturnData<GridInfo<server.Site>>>(ap.GET__api_Site, pm);
...
export async function callEdit(id) {
    ...
    let tm = { id };
    let pm = doWKPA(tm);
    let data = await ft<ReturnData<server.Site>>(ap.GET__api_Site_Item, pm);
    mask_off();
    if (data.state > 0) {
        alert(data.message);
        return;
...
export async function callRemove(id, p) {
    ...
    let tm = { id };
    let pm = doWKPA(tm);
    let data = await ft<ReturnBase>(ap.POST__api_Site_Remove, pm);
    mask_off();
    if (data.state > 0) {
...
export async function submitData(id: string | number, edit_type: IEditType, data: server.Site) {
    ...
    if (edit_type == IEditType.modify) {
        ...
        let data = await ft<ReturnUpdate<any>>(ap.POST__api_Site_Update, pm);
        ...
        let data = await ft<ReturnUpdate<any>>(ap.POST__api_Site, pm);
...
export const addState = () => {
    let r: { type: string, data: server.Site } = {
        type: ac.insert,
        data: { //新增值初始化
            site_id: 0
        }
...
//reduces
const page_grid = (state: GridInfo<server.Site> = { rows: [] }, action): GridInfo<server.Site> => {
    switch (action.type) {
        case ac.load:
            return action.data;
        case ac.page:
...
const field = (state = {}, action): server.Site => {
    switch (action.type) {
        case ac.insert:
            return action.data
        case ac.modify:
...
const kfield = (state = {}, action): server.Site => {
    switch (action.type) {
        case ac.insert:
            return action.data;
        case ac.modify:
