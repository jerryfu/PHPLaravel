...
interface GridProps {
    ...
    edit_type?: IEditType,
    menu_name?: string,
    page_grid?: GridInfo<server.Site>,
    callEdit?: Function,
    callRemove?: Function,
    callPage?: Function,
    callLoad?: Function,
        ...
        out_html = <div style={{ display: show }}>
            ...
            <table className="table-list table-hover table-striped">
                ...
                <tbody>
                    {
                        row_data.map((item, i) => {
                            return <tr key={item.site_id}>
                                <td className="item-edit"><PWButton className="hover-danger oi" title={lang.delete} dataGlyph="trash" onClick={this.callRemove.bind(this, item.site_id)}></PWButton></td>
                                <td className="item-edit"><PWButton className="hover-success oi" title={lang.modify} dataGlyph="pencil" onClick={this.callEdit.bind(this, item.site_id)}></PWButton></td>
...
class Edit extends React.Component<EditProps, StateProps>{
    ...
    componentDidMount() {
        //ft<server.ProdKind[]>(ap.GET__api_Site).then((data) => {
