...
export async function callLoad(p) {
    ...
    let pm: PBase = {};
    let data = await ft<ReturnData<GridInfo<server.PlayDivide>>>(ap.GET__api_PlayDivide, pm);
...
export async function callEdit(id) {
    ...
    let tm = { id };
    let pm = doWKPA(tm);
    let data = await ft<ReturnData<server.PlayDivide>>(ap.GET__api_PlayDivide_Item, pm);
    mask_off();
    if (data.state > 0) {
        alert(data.message);
        return;
...
export async function callRemove(id, p) {
    ...
    let tm = { id };
    let pm = doWKPA(tm);
    let data = await ft<ReturnBase>(ap.POST__api_PlayDivide_Remove, pm);
    mask_off();
    if (data.state > 0) {
...
export async function submitData(id: string | number, edit_type: IEditType, data: server.PlayDivide) {
    ...
    if (edit_type == IEditType.modify) {
        ...
        let data = await ft<ReturnUpdate<any>>(ap.POST__api_PlayDivide_Update, pm);
        ...
        let data = await ft<ReturnUpdate<any>>(ap.POST__api_PlayDivide, pm);
...
export const addState = () => {
    let r: { type: string, data: server.PlayDivide } = {
        type: ac.insert,
        data: { //新增值初始化
            play_divide_id: 0
        }
...
export async function editTeam(play_divide_id) {
    var data = await ft<ReturnData<server.Team[]>>(ap.GET__api_PlayDivideTeam, { play_divide_id })
    let r = {
        type: ac.editTeam,
        data: data.data,
        play_divide_id
...
export function setSelectTeam(data) {

    return {
        type: ac.setSelectTeam,
        data
    }
}
export function sortTeam(data) {
...
//reduces
const page_grid = (state: GridInfo<server.PlayDivide> = { rows: [] }, action): GridInfo<server.PlayDivide> => {
    switch (action.type) {
        case ac.load:
            return action.data;
        case ac.page:
...
const field = (state = {}, action): server.PlayDivide => {
    switch (action.type) {
        case ac.insert:
            return action.data
        case ac.modify:
...
const kfield = (state = {}, action): server.PlayDivide => {
    switch (action.type) {
        case ac.insert:
            return action.data;
        case ac.modify:
...
const set_play_team = (state = {}, action) => {

    switch (action.type) {
        case ac.editTeam:
            //console.log('check log', action.data)
            return action.data;
        case ac.setSelectTeam:
            return action.data;
        case ac.sortTeam:
            return action.data;
        case ac.sortRedo:
...
export let store = cbnReduce({ menudata, oper_id, search, page_grid, edit_type, view_mode, field, kfield, exist, set_play_team });